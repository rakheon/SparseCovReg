...
#covl_lambmin = numeric(num_simu); covl_almin = numeric(num_simu)     # list of selected tuning parameters, lambda and alpha, by cross validation (Loading the cross validation result as above will give these lists)
S_diff = numeric(num_simu); ST_diff = S_diff; lse_diff = S_diff; covl_diff = S_diff; covlpd_diff = S_diff; covreg_diff = S_diff     # storage for summarizing individual covariance matrix estimation error
covl_err_est = S_diff                                # storage for regression coefficient estimation error
covl_err_pred = S_diff                               # storage for prediction error
lasso_TPR = numeric(num_simu); lasso_FPR = numeric(num_simu)    # storages for true positive rate and false positive rate
coef_evalmin = matrix(NA, nrow=num_simu, ncol=q+1)   # storage for minimum eigenvalue (will be used for PD adjustment)
Sigma_diag = matrix(NA, nrow = q+1, ncol = p); Sigma_off = matrix(NA, nrow = q+1, ncol = p*(p-1)/2)    # matrix for storing the true covariance matrix entries
D <- duplication.matrix(p)                           # duplication matrix
Dp = solve(t(D)%*%D)%*%t(D)                          # moore-penrose inverse of the duplication matrix
diag_id = (vech(diag(p))==1)                         # identifier for diagonal elements
tb_COV = matrix(NA, nrow = num_simu, ncol = 6)       # storage for coverage ratio in inference
Sigma_12_mat = matrix(NA, nrow=num_simu, ncol = n)   # storage for Figure 1 in the manuscript
S_12_mat = Sigma_12_mat; covreg_12_mat = Sigma_12_mat; lse_12_mat = Sigma_12_mat; covl_12_mat = Sigma_12_mat    # storage for Figure 1 in the manuscript
...
for (ii in 1:num_simu){
  ...
  Sigma_coeftot = matrix(NA, nrow=q+1, ncol=p*(p+1)/2)            # vectorization of the true population-level covariance matrix and the true covariate effects
  for (iii in 1:(q+1)){
    Sigma_coeftot[iii,] = vech(Sigma_array[,,iii])
  }
  Sigma_coef0 = Sigma_coeftot[,diag_id]                           # diagonals of the true population-level covariance matrix and the true covariate effects
  Sigma_coef = Sigma_coeftot[,!diag_id]                           # vectorization of off-diagonals of the true population-level covariance matrix and the true covariate effects
  ...
  covreg_indiv=array(0, c(p,p,n))                     # list of estimated subject-specific covariance matrices by Hoff and Niu (2012)
  ...
  #covl_lambmin[ii] = covl_cv2$lambda_min; covl_almin[ii] = covl_cv2$alpha_min            # list of selected tuning parameters, lambda and alpha, by cross validation (Loading the cross validation result as above will give these lists)
  ...
  XZZ=XZ; XZZ[,-1]=z
  covl_err_est[ii] = sum(((covl_beta-Sigma_coef))^2)+sum(((covl_beta0-Sigma_coef0))^2)                   # regression coefficient estimation error         
  covl_err_pred[ii] = (sum((XZZ%*%(covl_beta-Sigma_coef))^2)+sum((XZZ%*%(covl_beta0-Sigma_coef0))^2))/n  # prediction error
  ...
  true_betaU_975 = covl_betaU + moe          # the 95% confidence interval upper bound of off-diagonals with true variance of W
  true_betaU_025 = covl_betaU - moe          # the 95% confidence interval lower bound of off-diagonals with true variance of W
...
########## Code for Figure 1 (for MA model; Setting 1; n=500; q=30) ############
...
########## Code for Figure 2 (for MA model; Setting 2; n=500; q=100) ############
